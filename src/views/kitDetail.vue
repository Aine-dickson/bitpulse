<script setup lang="ts">
    ...
    // Count fields sit above the tab bar and stay on screen whichever panel is
    // open: the quantity is the number people keep going back to adjust, so hiding
    // it behind a tab would mean constant switching.
    const countFields = computed(() => (kit.value?.fields ?? []).filter((f) => f.kind === 'count'))

    // Tabs, in first-appearance order. Fields sharing a `tab` value share a panel;
    // a field without one gets its own, titled with its label.
    const tabs = computed(() => {
        const out: { label: string; fields: KitField[] }[] = []
        for (const f of kit.value?.fields ?? []) {
            if (f.kind === 'count') continue
            const label = f.tab ?? f.label
            const existing = out.find((t) => t.label === label)
    ...
    const activeTab = ref('')
    // Keep the selection valid when the route moves to a different kit.
    watch(
        tabs,
        (t) => {
            if (!t.some((x) => x.label === activeTab.value)) activeTab.value = t[0]?.label ?? ''
        },
        { immediate: true },
    )

    const currentFields = computed(
        () => tabs.value.find((t) => t.label === activeTab.value)?.fields ?? [],
    )

    /** Which tabs hold a field the current configuration has a problem with. */
    const flaggedTabs = computed(() => {
        const ids = new Set((quote.value?.warnings ?? []).map((w) => w.fieldId))
        return new Set(
            tabs.value.filter((t) => t.fields.some((f) => ids.has(f.id))).map((t) => t.label),
        )
    })
    ...
    <div class="field">
        ...
        <template v-else>
            ...
            <section class="bg-paper">
                <div class="mx-auto max-w-[1000px] px-5 pb-8 pt-8 sm:px-8 sm:pb-10 sm:pt-12">
                    ...
                    <div class="mt-5 flex items-start gap-4">
                        ...
                        <div>
                            <h1
                                class="text-[1.65rem] font-extrabold leading-[1.08] tracking-tight text-inkf sm:text-[2.4rem]">
                                {{ kit.name }}
                            </h1>
            ...
            <section class="border-t border-rule bg-card">
                <div
                    class="mx-auto grid max-w-[1000px] gap-8 px-5 py-10 sm:px-8 sm:py-12 lg:grid-cols-[1.15fr_0.85fr] lg:items-start">
                    <!-- controls -->
                    <div class="flex flex-col gap-7">
                        <!-- Quantity stays put above the tabs -->
                        <div v-for="f in countFields" :key="f.id">
                            ...
                            <div class="mt-3 flex items-center gap-3">
                                <button type="button" class="step-btn" :disabled="Number(config[f.id]) <= f.min"
                                    :aria-label="`Fewer ${f.unitLabel}`" @click="bump(f.id, -1, f.min, f.max)">
                                    −
                                </button>
                                <input :id="`f-${f.id}`" v-model.number="config[f.id]" type="number"
                                    class="fld num max-w-[110px] text-center" :min="f.min" :max="f.max"
                                    inputmode="numeric" />
                                <button type="button" class="step-btn" :disabled="Number(config[f.id]) >= f.max"
                                    :aria-label="`More ${f.unitLabel}`" @click="bump(f.id, 1, f.min, f.max)">
                                    +
                                </button>
                        ...
                        <!-- Tab bar: keeps the rest to one panel at a time instead of a
                        long scroll, which matters most on a phone. -->
                        <div class="tabs" role="tablist" aria-label="Configuration sections">
                            <button v-for="t in tabs" :key="t.label" type="button" role="tab" class="tab"
                                :class="{ 'tab-on': activeTab === t.label, 'tab-flag': flaggedTabs.has(t.label) }"
                                :aria-selected="activeTab === t.label" @click="activeTab = t.label">
                                {{ t.label }}
                            </button>
                        </div>
                        ...
                        <div v-for="f in currentFields" :key="f.id">
                            ...
                            <div v-if="f.kind === 'choice'" class="mt-3 grid gap-2">
                                <label v-for="c in f.choices" :key="c.id" class="seg"
                                    :class="{ 'seg-on': config[f.id] === c.id }">
                                    <input v-model="config[f.id]" type="radio" :name="f.id" :value="c.id" />
                                    <span class="flex-1">
                                    ...
                                    <span class="num shrink-0 text-[0.85rem] font-semibold text-muted">
                                        <template v-if="c.price > 0">
                                            +{{ c.price.toLocaleString('en-US') }}<template
                                                v-if="f.multiplyBy">/ea</template>
                                        </template>
                            ...
                            <!-- toggle -->
                            <div v-else-if="f.kind === 'toggle'" class="mt-3">
                    ...
                    <aside class="lg:sticky lg:top-6">
                        <div class="rounded-xl border border-rule bg-paper p-5 sm:p-6">
                            ...
                            <ul v-if="quote!.warnings.length"
                                ...
                                <li v-for="w in quote!.warnings" :key="w.fieldId" class="flex gap-2">
            ...
            <section class="border-t border-rule bg-card">
                <div class="mx-auto max-w-[1000px] px-5 py-10 sm:px-8 sm:py-12">
                    ...
                    <div class="mt-5 grid gap-3 sm:grid-cols-2">
                        <RouterLink v-for="o in others" :key="o.slug" :to="`/kits/${o.slug}`"
            ...
            <footer class="border-t border-rule bg-paper pb-[calc(76px+env(safe-area-inset-bottom))] md:pb-0">
                <div class="mx-auto flex max-w-[1000px] flex-wrap items-center justify-between gap-3 px-5 py-7 sm:px-8">
                    ...
                    <RouterLink to="/" class="text-[0.8rem] text-muted underline underline-offset-4">BitPulse
                    </RouterLink>
                </div>
            </footer>
